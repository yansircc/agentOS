...
const collectFailures = (root = repoRoot) => {
  ...
  if (!exists(root, rootPackagePath)) {
    failures.push(`${rootPackagePath}: missing root package manifest`);
    return failures;
  }
  const rootPackage = readJson(root, rootPackagePath);
  if (
    rootPackage.scripts?.["test:resource-quota-protocol-ownership"] !==
    "node scripts/check-resource-quota-protocol-ownership.mjs --self-test && node scripts/check-resource-quota-protocol-ownership.mjs"
  ) {
    failures.push(`${rootPackagePath}: missing test:resource-quota-protocol-ownership script`);
  }
