...
| `@agent-os/runtime`   | yes       | 0.5.x public | runtime root exports only backend-neutral ledger/projection/service algebra. Platform/provider, testing, and wire-projection integrations live behind explicit subpath exports: ./ag-ui, ./channel, ./cloudflare, ./cloudflare/do-rpc, ./in-memory, ./local, ./node, ./llm-effect-ai, ./telemetry-otlp, and ./testing; root JS and root declarations must not leak subpath-only peers. The ./ag-ui subpath is a browser-safe projection over runtime-protocol Recorded vocabulary, not the client state model. The ./channel subpath is a pure authored ingress contract for generated targets: define routes, verify provider requests into principals, and receive a restricted submit/dispatch context. It is distinct from attached-stream outbound frame channels and must not own provider SDK lifecycle, webhook helpers, durable deduplication, provider-normalized events, or AgentManifest facts. |
