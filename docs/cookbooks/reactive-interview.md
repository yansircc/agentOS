...
| `openai-chat-compatible` route via explicit `ProviderRegistry` works for BYOK routes | INV-8 means no ambient credentials; credentials are explicit route dependencies via `credentialRef`. |
