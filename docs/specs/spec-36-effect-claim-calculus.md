...
Boundary package survival is generated by five independent axes:

```text
Boundary = Vocabulary x Authority x Material x Proof x Projection
```

Core defines the calculus and `BoundaryContract`. Carrier packages declare a
provider-neutral boundary over those axes. Backend packages bind provider
material. Tooling reads projections. A package with no independent boundary
generator collapses into core or tooling; a provider-neutral carrier must not
be named as a provider backend.

Cleanup remains proof/lifecycle vocabulary in v0: `destroyed`, `reaped`, and
similar facts settle a claim with symbolic cleanup proof. If cleanup later
changes claim identity or requires independent authority across N>=2 products,
this placement law must reopen a sixth axis instead of hiding cleanup under
proof.
...
| ------------------ | -------- | ----------------------------------- | --------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| run-stream         | package  | N=1 (vibe)                          | `@agent-os/run-stream` composes ledger rows, optional `turn-stream` frames, and terminal `submit` results into consumer frames; the batched bridge is explicitly non-realtime, the realtime composer consumes caller-provided live sources, ledger rows remain durable truth, and turn frames remain non-durable UI/progress data |
...
| context pack       | core/app | N≥2 (vibe, zeroY)                   | `@agent-os/core/context` owns deterministic fact packing and redaction spans because it is pure reader algebra with no external boundary; product context selection and summarization strategy remain app-owned                                                                                                                   |
...
combinations: vibe uses Cloudflare Workflow + D1 + ledger, while zeroY uses
...
| MCP registry          | Two products share the same discovery, install, authority, and material contract; sharing only MCP client usage is not enough                           |
...
| skill-registry                           | adapter     | install-time tool generator          | N=0               | `@agent-os/skill-registry` validates skill manifests and constructs registered tools from package-owned definitions; skill identity stops at install time, and runtime sees only `ToolContract` values with no skill ledger/event/projection vocabulary     |
...
| git/deploy/staging/verification carriers | backend     | generator/resolver as needed         | N=1 (vibe)        | declare provider-neutral `BoundaryContract` values and adopt claim settlement/proof anchors without app-specific nouns such as `changeId`; `@agent-os/deploy` owns deploy vocabulary, while provider-specific deploy backends remain separate               |
...
| context pack                             | core        | reader                               | N≥2 (vibe, zeroY) | `@agent-os/core/context` derives prompt context from supplied facts with explicit redaction spans and included/omitted refs; product context strategy stays app-owned                                                                                       |
