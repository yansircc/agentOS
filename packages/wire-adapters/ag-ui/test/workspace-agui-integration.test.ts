...
const runToolEffect = <A>(effect: Effect.Effect<A, unknown, unknown>): Promise<A> =>
...
describe("Cloudflare workspace tools to AG-UI integration fixture", () => {
  it("uses standard workspace binding, receipt-backed writes, diagnostics, and safe AG-UI egress", async () => {
    ...
    const mkdir = vi.fn<NonNullable<CloudflareWorkspaceEnvClient["mkdir"]>>(async () => undefined);
    ...
    const readResult = await runToolEffect(
      readTool.execute({ path: "/src/input.txt" }, { materials: {} }),
    );
    ...
    const writeResult = await runToolEffect(
      writeTool.execute(
    ...
    expect(writeFile).toHaveBeenCalledWith("/workspace/project/src/output.txt", "WRITE_SECRET", {
      encoding: "utf-8",
    });
