  ...
  it.effect("settles reservations by consuming success and releasing failure", () =>
    Effect.gen(function* () {
      ...
      const success = yield* withImageResourceSettlement(
        ...
        {
          consume: (value) =>
            Effect.sync(() => successMarks.push(`consume:${value}`)),
          release: (error) =>
            Effect.sync(() => successMarks.push(`release:${String(error)}`)),
        },
      );
      ...
      const failure = yield* Effect.flip(
        withImageResourceSettlement(Effect.fail("bad"), {
          consume: (value) =>
            Effect.sync(() => failureMarks.push(`consume:${value}`)),
          release: (error) =>
            Effect.sync(() => failureMarks.push(`release:${error}`)),
        }),
      );
