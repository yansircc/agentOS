...
export const InMemoryTriggerPumpLive = (
  ...
  Layer.effect(
    TriggerPump,
    Effect.withSpan("agentos.in_memory.trigger_pump.live")(
      Effect.gen(function* () {
        ...
        const drainDue = (now: number) =>
          Effect.gen(function* () {
            ...
            return { drained };
          }).pipe(Effect.withSpan("agentos.in_memory.trigger_pump.drain_due"));
        const cancelTrigger = (spec: {
          ...
          }).pipe(Effect.withSpan("agentos.in_memory.trigger_pump.cancel_trigger"));
        return {
          drainDue,
          drainUntilQuiet: (now, options) => drainTriggerPumpUntilQuiet(drainDue, now, options),
          cancelTrigger,
          stuckTriggers: (now) =>
            Effect.succeed({ stuck: state.stuckDueWork(identity, now) }).pipe(
              Effect.withSpan("agentos.in_memory.trigger_pump.stuck_triggers"),
            ),
        };
      }),
    ),
