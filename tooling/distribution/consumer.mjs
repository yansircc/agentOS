...
export const writeConsumerApp = (dir, extraDeps = {}) => {
  ...
  fs.writeFileSync(
    ...
    [
      ...
      "const runTypedExternalEffect = defineExternalEffectAttempt();",
      "const externalEffectProjection = await Effect.runPromise(runTypedExternalEffect({",
      "  spec: { value: 'packed' },",
      ...
      "  runScenario: (scenario) => Effect.succeed({",
      "    scenarioId: scenario.id,",
      "    status: 'passed',",
      "    evidence: {",
      "      observations: Object.fromEntries(scenario.requiredObservations.map((name) => [name, true])),",
      "    },",
      "  }),",
      "}));",
