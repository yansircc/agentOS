...
const collectManifestFailures = (root, failures) => {
  failures.push(
    ...collectBoundaryRuleMembershipFailures(root, [
      {
        ruleId: "node-postgres-effect-governance",
...
const writePositiveFixture = (root) => {
  ...
  writeJsonFixture(root, "docs/agent/boundary-rules.source.json", {
    schemaVersion: 1,
    commandGroups: {
      all: [{ type: "group", id: "substrate-consumer" }],
      "substrate-consumer": [{ type: "rule", id: "node-postgres-effect-governance" }],
    },
    rules: [
      {
        id: "node-postgres-effect-governance",
        commandGroup: "substrate-consumer",
      },
    ],
  });
