...
const regularFailures = () => {
  ...
  const boundaryRules = readJson("docs/agent/boundary-rules.source.json");
  const boundaryRuleCommands = Object.fromEntries(
    (Array.isArray(boundaryRules.rules) ? boundaryRules.rules : []).map((rule) => [
      rule.id,
  ...
  const projection = buildCapabilityRouteProjection({
    ...
    rootScripts: { ...model.rootScripts, ...boundaryRuleCommands },
